(() => {
  ...
  const debug = app.debug = app.debug || {};

  /**
   * Optional runtime debug surface. Loaded always for now (no build flag), but
   * isolated from the main content-script control flow so feature code stays readable.
   */
  function attachRuntimeDebug(deps) {
    const adapter = deps.adapter;
    const settingsStore = deps.settingsStore;
    const subtitleStore = deps.subtitleStore;
    const translationQueue = deps.translationQueue;
    const databaseClient = deps.databaseClient;
    const languageUtils = deps.languageUtils || app.languageUtils;
    const extensionApi = deps.extensionApi || app.extensionApi;
    const logRuntime = typeof deps.logRuntime === 'function'
      ? deps.logRuntime
      : function logRuntime(stage, detail) {
        if (extensionApi && extensionApi.debugLog) {
          extensionApi.debugLog.record('runtime', stage, detail);
        }
      };
    ...
    function getTranslationLogPayload() {
      return {
        ...
        entries: extensionApi && extensionApi.debugLog
          ? extensionApi.debugLog.list()
          : []
      };
    }
...
globalThis.__NLL_DEBUG__ = {
  ...
  saveRecentVisibilityTrace(windowMs = 5000) {
    ...
    return {
      destroy() {
        if (rollingVisibilityRecorder && typeof rollingVisibilityRecorder.stop === 'function') {
          rollingVisibilityRecorder.stop();
        }
        if (globalThis.__NLL_DEBUG__) {
          delete globalThis.__NLL_DEBUG__;
        }
      }
    };
  }

  debug.attachRuntimeDebug = attachRuntimeDebug;
})();
